...
import './App.css';
import { invokeCallGuardAI } from './services/edgeFunctions';
...
function Header({ now, highRiskCount, theme, onToggleTheme, onSignOut }) {
  return (
    <div className="header">
      ...
      <div className="headerRight">
        ...
        <button className="chipGhost clickable" onClick={onSignOut}>
          Sign out
        </button>
...
function LiveTestPanel({ onAddLog }) {
  ...
  const analyze = useCallback(async () => {
    ...
    setResult(null);
    try {
      const resp = await invokeCallGuardAI({ transcript });
      if (!resp.success) {
        throw new Error(resp.error || 'Failed to analyze transcript');
      }
      ...
      setError(e?.message || 'Could not call callguard-ai');
    } finally {
      setLoading(false);
    }
  }, [inputText, onAddLog]);
...
function Footer() {
  return (
    <div className="footer">
      ...
      <div>Powered by Supabase Edge Functions</div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  async function loadRealData() {
    ...
    const mapped = REAL_TEST_CALLERS.map((caller, idx) =>
      normalizeLog(
        {
          id: nowMs + idx,
          ...
          risk: idx === 0 ? 'SCAM' : idx === 1 ? 'SUSPICIOUS' : 'SAFE',
          reason: 'Sample data',
    ...
    window.setTimeout(() => {
      setLogs((prev) => prev.map((l) => (mapped.some((m) => m.id === l.id) ? { ...l, isNew: false } : l)));
    }, 1200);
  }
    ...
    <div className="appRoot">
      ...
      <div className="container">
        <div className="topActions">
          <button className="btnGhost" onClick={loadRealData}>
            Load Sample Data
          </button>
