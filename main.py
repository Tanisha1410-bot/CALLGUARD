...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    # We do not use cookies/auth headers, so disabling credentials
    # avoids browser CORS edge cases with wildcard origins.
...
def local_fallback_analysis(transcript: str) -> dict:
    """Simple keyword-based fallback when AI key is unavailable."""
    ...
    keyword_rules = [
        ("otp", "OTP request", 35),
        ("upi", "UPI transfer request", 20),
        ("bank details", "Bank details request", 20),
        ("account block", "Account block threat", 20),
        ("kyc", "KYC fraud tactic", 15),
        ("urgent", "Urgency tactic", 10),
        ("turant", "Urgency tactic", 10),
        ("lottery", "Reward scam", 20),
        ("kbc", "Reward scam", 20),
        ("rbi", "Authority impersonation", 15),
        ("sbi", "Bank impersonation", 15),
        ("cbi", "Authority impersonation", 15),
        ("police", "Fear/authority tactic", 15),
        ("warrant", "Legal threat", 20),
        ("jail", "Legal threat", 20),
    ]

    for needle, label, points in keyword_rules:
        if needle in text:
            flags.append(label)
            score += points
    ...
    if score >= 60:
        risk = "SCAM"
        reason = "Multiple scam signals detected in the transcript."
    elif score >= 25:
        risk = "SUSPICIOUS"
        reason = "Some scam-like signals were detected."
    else:
        risk = "SAFE"
        reason = "No strong scam indicators were detected."

    return {
        "risk": risk,
        "reason": reason,
        "confidence": max(score, 55 if risk != "SAFE" else 80),
...
def process_transcript(transcript: str, endpoint: str) -> dict:
    ...
    if not ANTHROPIC_API_KEY:
        parsed_json = local_fallback_analysis(transcript)
